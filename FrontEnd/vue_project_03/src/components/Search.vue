        ...
        <div class="item">
            ...
            <div class="container d-flex align-items-center h-center">
                <div class="wrap-caption">
                    <h1 class="caption-heading text-white">Deep insight and quality service</h1>
                        <el-tabs :stretch="true" v-model="activeName" @tab-click="handleClick">
                          <el-tab-pane label="企业/Enterprise" name="first"><search-box1 class="center search-box1" />
                          <div class="suggestion">
                            <span class="title">Try:</span>
                            <span class="content">
                            <div v-for="(suggestion, index) in search_suggestion1" :key="index">
                              <router-link :to="{path: '/retrieval', query:{query: suggestion}}" tag="a">{{suggestion}}</router-link>
                            </div>
                            </span>
                          </div></el-tab-pane>
                          <el-tab-pane label="行业/Industry" name="second"><search-box2 class="center search-box2" />
                          <div class="suggestion">
                          <span class="title">Try:</span>
                          <span class="content">
                          <div v-for="(suggestion, index) in search_suggestion2" :key="index">
                            <router-link :to="{path: '/industry', query:{query: suggestion}}" tag="a">{{suggestion}}</router-link>
                          </div>
                          </span>
                          </div></el-tab-pane>
                        </el-tabs>
...
export default {
  ...
  data () {
    return {
      search_suggestion1: ["蓝思科技"],
      search_suggestion2: ["玻璃","互联网"],
      activeName: 'first'
    }
  },
  methods: {
